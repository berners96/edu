							...
							<c:when test="${!empty list}">
								<c:forEach var="list" items="${list}">
									<tr>
										...
										<td class="title"><a href="#this" name="title" style="text-decoration:none"><c:out value="${list.boardSubject}" /></a>
										<input type="hidden" id="boardSeq" value="${list.boardSeq}"></td>
										<td><c:out value="${list.boardWriter}" /></td>
										<td><c:out value="${list.boardHits}" /> <!-- 원하는 날짜 형식으로 출력하기 위해 fmt태그 사용 -->
										<td><fmt:formatDate value="${list.insDate}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
